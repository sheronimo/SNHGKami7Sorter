...
function convertCSVArrayToTraineeData(csvArrays) {
	trainees = csvArrays.map(function (traineeArray, index) {
		...
		trainee.top16 = traineeArray[4] === 't'; // sets trainee to top 12 if 't' appears in 6th column
...
function populateTableEntry(trainee) {
	...
	let top16 = showTop16 && trainee.top16 && 'top16';
	const tableEntry = `
  <div class="table__entry ${eliminated}">
    <div class="table__entry-icon">
      <img class="table__entry-img" src="assets/trainees/${trainee.image}" />
      <div class="table__entry-icon-border ${trainee.grade.toLowerCase()}-team-border"></div>
      ${top16 ? '<div class="table__entry-icon-crown"></div>' : ''}
      ${
				trainee.selected
  ...
  <div class="ranking__entry ${eliminated}">
    <div class="ranking__entry-view">
      <div class="ranking__entry-icon">
        ...
        <div class="ranking__entry-icon-border ${trainee.grade.toLowerCase()}-team-border" data-rankid="${
		currRank - 1
	}"></div>
      </div>
      <div class="ranking__entry-icon-badge bg-${trainee.grade.toLowerCase()}">${currRank}</div>
      ${top16 ? '<div class="ranking__entry-icon-crown"></div>' : ''}
    </div>
...
const currentURL = 'https://sheronimo.github.io/SNHGTop16Sorter/';
...
const rowNums = [1, 2, 3, 4, 6];
//window.addEventListener("load", function () {
populateRanking();
readFromCSV('./member_info.csv');
//});
